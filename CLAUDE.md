...
B-01 delivered the skeleton: `package.json`, `tsconfig.json`, `next.config.ts`, `.nvmrc`, `.gitignore`, `app/layout.tsx`, `app/page.tsx`, a lock file, and a git repo on `main`. B-02 onward extends it — do not re-scaffold.
...
Project-local notes (not third-party) are in `docs/context/`: `clean-code-ts.md`, and a Next.js note on lazy-loading client components that bears on the client-only Leaflet mount in B-02.
...
Run `nvm use` first — it reads `.nvmrc` (Node 24), the one place the version is pinned. Playwright commands arrive with B-07.
...
Playwright's browser download is slow — run it in the background.
...
No real data, no fetch button, no server-side scaffolding, no "for later" folders, no example code from `create-next-app`, no extra dependencies. Real AIS data is a later release; do not prepare for it.
