...
export const CHECKS = [
  ...
  },
  {
    ...
    proves:
      ...
      + 'словом і `from` стоїть коментар із крапкою з комою',
    ...
    blindSpot:
      ...
      + 'Забілення коментарів для другого проходу рядкових літералів не розуміє, тож '
      + '`//` у літералі з\'їдає решту СВОГО рядка: код і початок багаторядкового '
      + 'імпорту на ОДНОМУ рядку ховають цей імпорт від обох проходів. У периметрі '
      + 'таких рядків нуль, і жоден лінтер такого стилю не пропустить',
  },
  {
    id: 'build',
    tier: 'full',
