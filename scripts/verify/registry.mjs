...
export const CHECKS = [
  ...
  },
  {
    ...
    blindSpot:
      ...
      + 'Сканер не парсить мову, а зіставляє форми порядково: специфікатор, розбитий '
      + 'на два рядки, не збігається з жодною з семи форм. '
      + 'Периметр сканування збігається з периметром хеша свіжості рядок у рядок '
      + '(hash.mjs:SOURCE_PREFIXES), і тримається це домовленістю, а не механізмом: '
      + 'розійдуться списки — файл почне міняти вердикт цієї перевірки, не міняючи '
      + 'ключа свіжості, і `--reuse-if-fresh` віддасть старе зелене',
  },
  {
    id: 'build',
    tier: 'full',
