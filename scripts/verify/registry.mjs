...
export const CHECKS = [
  ...
  },
  {
    ...
    after: [],
    proves:
      'Жоден файл із периметра свіжості (`hash.mjs:isSourcePath`), що має розширення '
      + 'коду або `.css`, не містить статичного `import`/`export … from`/`require`/'
      + '`@import` (ЛИШЕ в лапках) / `/// <reference path>` зі шляхом, у якому є '
      + 'сегмент `reference` — '
      + 'ні в рантаймі, ні на рівні типів, ні в багаторядковій формі',
    ...
    blindSpot:
      ...
      + 'Двох форм немає в сімці й вони пройдуть мовчки: `@import url(…)` БЕЗ лапок '
      + '(валідний CSS, а `SPECIFIER` вимагає лапок) і `/// <reference types="…" />` '
      + '(ловиться лише варіант із `path=`). '
      + 'Число порушень уміє занижувати: ключ дедуплікації — рядок плюс специфікатор, '
      + 'тож ДВА однакові специфікатори в одному рядку дають один рядок звіту. '
      + 'Вердикт від цього не міняється ніколи (нуль проти не-нуля), лише N. '
      + 'У зворотний бік сканер свідомо перебирає: він не парсить мову й не вирізає '
      + 'коментарі, тож проза, яка саме правило ЦИТУЄ, здатна дати хибне спрацювання. '
      + 'Це вибір «перебір безпечний, недобір — ні», а не недогляд. '
