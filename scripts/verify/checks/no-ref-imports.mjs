...
export function scanText(relPath, text) {
  const violations = [];
  // Одне порушення вміє збігтися з кількома формами. Ключ — трійка
  ...
  // що й нижче: перебір безпечний, недобір — ні. Зайвий рядок звіту бачить
  // людина; зниклий не бачить ніхто.
  const seen = new Set();
