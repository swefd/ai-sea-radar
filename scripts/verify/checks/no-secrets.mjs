...
export function scanLine(line, stats = null) {
  ...
  for (const pattern of PATTERNS) {
    ...
    // його — сюїта лишається зеленою (89 passed). Причина в тому, що `exec` на
    // глобальному регексі, не знайшовши збігу, САМ скидає `lastIndex` у нуль, а
    // цикл нижче завжди доходить до цього null. Виміряно: після вичерпаного циклу
    // `lastIndex` дорівнює 0; після ОДНОГО `exec` — 4, тобто ненульовий.
    //
    // Отже рядок стає несучим рівно в одній ситуації: якщо цикл перестане
    // вичерпуватися — наприклад, `while` нижче замінять на `if`. Саме цю заміну
    // ловить тест «scanLine бачить УСІ збіги рядка», тож пара тримається: єдина
    // зміна, що робить це скидання потрібним, уже червонить сюїту. Рядок лишається
    // як варта на випадок такої зміни, але видавати його за доведений не можна.
...
function main() {
  ...
  // варта й потрібна.
  //
  // Код 2, не 1. `classifyExit` (run.mjs:113) мапить обидва у FAILED, тож блокує
