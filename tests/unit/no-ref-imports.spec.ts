...
test('blindSpot: названі діри справді є — і почервоніють, коли їх закриють', () => {
  ...
  // Межа забілення, що розуміє літерали: вкладений шаблон `${…}` з бектиком
  // усередині здатен спарувати лапки не так, і тоді маркер, який насправді
  // всередині літерала, читається як код. Щоб із цього вийшов недобір, потрібні
  // ще й код і ПОЧАТОК багаторядкового імпорту на ОДНОМУ рядку. У периметрі
  // вкладених шаблонів нуль зі 119 інтерполяцій (виміряно парсером TypeScript).
  expect(found([
